...
groups.each{ group, files ->
    ...
    if (group.anime == null && subtitles != null && files.findAll{ it.isVideo() }.size() > 0) {
        subtitles.each{ languageCode ->
            def subtitleFiles = getMissingSubtitles(file:files, lang:languageCode, strict:true, output:'srt', encoding:'ISO-8890-1', db: 'OpenSubtitles', format:'MATCH_VIDEO_ADD_LANGUAGE_TAG') ?: []
            files += subtitleFiles
